...
function New-XxXInMemoryModule
{
<#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        'struct', and Add-XxXWin32Type functions.
    ...
    .EXAMPLE
        ...
        $Module = New-XxXInMemoryModule -ModuleName Win32
#>
...
# prototypes for Add-XxXWin32Type.
function func
{
    Param
    (
...
<#
    ...
    .DESCRIPTION
        ...
        Add-XxXWin32Type enables you to easily interact with unmanaged (i.e.
        Win32 unmanaged) functions in PowerShell. After providing
        Add-XxXWin32Type with a function signature, a .NET type is created
    ...
    .PARAMETER Module
        ...
        New-XxXInMemoryModule to define an in-memory module.

    .PARAMETER Namespace

        An optional namespace to prepend to the type. Add-XxXWin32Type defaults
        to a namespace consisting only of the name of the DLL.

    .EXAMPLE

        $Mod = New-XxXInMemoryModule -ModuleName Win32
        ...
        $Types = $FunctionDefinitions | Add-XxXWin32Type -Module $Mod -Namespace 'Win32'
    ...
    .NOTES
        ...
        Add-XxXWin32Type with an array of function signatures. That way, they
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXIPAddress -ComputerName SERVER
        
        Return the IPv4 address of 'SERVER'

    .EXAMPLE

        PS C:\> Get-Content .\hostnames.txt | Get-XxXIPAddress
...
#>
    ...
    try {
        ...
        $Computer = $ComputerName | Get-XxXNameField
...
#>
    ...
    elseif(-not $Domain) {
        $Domain = (Get-XxXNetworkDomain).Name
    }
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetUser jason | select useraccountcontrol | ConvertFrom-UACValue

        Convert the UAC value for 'jason' to human readable format.

    .EXAMPLE

        PS C:\> Get-XxXNetUser jason | select useraccountcontrol | ConvertFrom-UACValue -ShowAll
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetUser -SPN | Request-SPNTicket
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXPathAcl "\\SERVER\Share\" 
        ...
        try {
            ...
            $ACL.GetAccessRules($true,$true,[System.Security.Principal.SecurityIdentifier]) | ForEach-Object {
                ...
                if ($_.IdentityReference -match '^S-1-5-21-[0-9]+-[0-9]+-[0-9]+-[0-9]+') {
                    $Object = Get-XxXADObject -SID $_.IdentityReference
                    $Names = @()
                    $SIDs = @($Object.objectsid)

                    if ($Recurse -and (@('268435456','268435457','536870912','536870913') -contains $Object.samAccountType)) {
                        $SIDs += Get-XxXNetGroupMember -SID $Object.objectsid | Select-Object -ExpandProperty MemberSid
                    }
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetComputer -FullData | Get-XxXNameField
#>
    [CmdletBinding()]
    param(
    ...
    elseif($Object) {
        if ( [bool]($Object.PSobject.Properties.name -match "dnshostname") ) {
            # objects from Get-XxXNetComputer
            $Object.dnshostname
        }
        elseif ( [bool]($Object.PSobject.Properties.name -match "name") ) {
            # objects from Get-XxXNetDomainController
            $Object.name
        }
        else {
...
<#
    .SYNOPSIS
        ...
        Used by several of the Get-XxXNetwork* function.
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXDomainSearcher -Domain testlab.local

    .EXAMPLE

        PS C:\> Get-XxXDomainSearcher -Domain testlab.local -DomainController SECONDARY.dev.testlab.local
#>
    ...
    if(-not $Credential) {
        ...
        elseif(-not $DomainController) {
            try {
                ...
                $DomainController = ((Get-XxXNetworkDomain).PdcRoleOwner).Name
            }
            catch {
                throw "Get-XxXDomainSearcher: Error in retrieving PDC for current domain"
            }
        }
    }
    elseif (-not $DomainController) {
        ...
        try {
            $DomainController = ((Get-XxXNetworkDomain -Credential $Credential).PdcRoleOwner).Name
        }
        catch {
            throw "Get-XxXDomainSearcher: Error in retrieving PDC for current domain"
        }
    ...
    $SearchString += $DN
    Write-Verbose "Get-XxXDomainSearcher search string: $SearchString"
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXDNSZone -Domain dev.testlab.local -DomainController primary.testlab.local
...
#>
    ...
    $DNSSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -PageSize $PageSize -Credential $Credential
    ...
    $DNSSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -PageSize $PageSize -Credential $Credential -ADSprefix "CN=MicrosoftDNS,DC=DomainDnsZones"
    $DNSSearcher.filter="(objectClass=dnsZone)"
...
<#
    ...
    .PARAMETER ZoneName
        ...
        The zone to query for records (which can be enumearted with Get-XxXDNSZone). Required.
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXDNSRecord -ZoneName testlab.local

        Retrieve all records for the testlab.local zone.

    .EXAMPLE

        PS C:\> Get-XxXDNSZone | Get-XxXDNSRecord

        Retrieve all records for all zones in the current domain.

    .EXAMPLE

        PS C:\> Get-XxXDNSZone -Domain dev.testlab.local | Get-XxXDNSRecord -Domain dev.testlab.local
...
#>
    ...
    $DNSSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -PageSize $PageSize -Credential $Credential -ADSprefix "DC=$($ZoneName),CN=MicrosoftDNS,DC=DomainDnsZones"
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetworkDomain -Domain testlab.local

    .EXAMPLE

        PS C:\> "testlab.local" | Get-XxXNetworkDomain
...
#>
    ...
    if($Credential) {
        ...
        Write-Verbose "Using alternate credentials for Get-XxXNetworkDomain"
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetworkForest -Forest external.domain

    .EXAMPLE
    
        PS C:\> "external.domain" | Get-XxXNetworkForest
#>
    ...
    if($Credential) {
        ...
        Write-Verbose "Using alternate credentials for Get-XxXNetworkForest"
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetForestDomain

    .EXAMPLE

        PS C:\> Get-XxXNetForestDomain -Forest external.local
#>
    ...
    $ForestObject = Get-XxXNetworkForest -Forest $Forest -Credential $Credential
...
filter Get-XxXNetForestCatalog {
<#
    .SYNOPSIS
...
filter Get-XxXNetDomainController {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetDomainController -Domain 'test.local'
        
        Determine the domain controllers for 'test.local'.

    .EXAMPLE

        PS C:\> Get-XxXNetDomainController -Domain 'test.local' -LDAP

        Determine the domain controllers for 'test.local' using LDAP queries.

    .EXAMPLE

        PS C:\> 'test.local' | Get-XxXNetDomainController
...
#>
    ...
    if($LDAP -or $DomainController) {
        ...
        Get-XxXNetComputer -Domain $Domain -DomainController $DomainController -Credential $Credential -FullData -Filter '(userAccountControl:1.2.840.113556.1.4.803:=8192)'
    }
    else {
        $FoundDomain = Get-XxXNetworkDomain -Domain $Domain -Credential $Credential
        if($FoundDomain) {
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetUser -Domain testing

    .EXAMPLE

        PS C:\> Get-XxXNetUser -ADSpath "LDAP://OU=secret,DC=testlab,DC=local"
#>
    ...
    begin {
        ...
        $UserSearcher = Get-XxXDomainSearcher -Domain $Domain -ADSpath $ADSpath -DomainController $DomainController -PageSize $PageSize -Credential $Credential
    }
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Add-XxXNetUser -UserName john -Password 'Password123!'
        
        Adds a localuser 'john' to the local machine with password of 'Password123!'

    .EXAMPLE

        PS C:\> Add-XxXNetUser -UserName john -Password 'Password123!' -ComputerName server.testlab.local
        
        Adds a localuser 'john' with password of 'Password123!' to server.testlab.local's local Administrators group.

    .EXAMPLE

        PS C:\> Add-XxXNetUser -UserName john -Password password -GroupName "Domain Admins" -Domain ''
    ...
    .EXAMPLE
        ...
        PS C:\> Add-XxXNetUser -UserName john -Password password -GroupName "Domain Admins" -Domain 'testing'
...
#>
    ...
    if ($Domain) {
        ...
        $DomainObject = Get-XxXNetworkDomain -Domain $Domain
        if(-not $DomainObject) {
    ...
    # if a group is specified, invoke Add-XxXNetGroupUser and return its value
    if ($GroupName) {
        # if we're adding the user to a domain
        if ($Domain) {
            Add-XxXNetGroupUser -UserName $UserName -GroupName $GroupName -Domain $Domain
            "[*] User $UserName successfully added to group $GroupName in domain $Domain"
        }
        # otherwise, we're adding to a local group
        else {
            Add-XxXNetGroupUser -UserName $UserName -GroupName $GroupName -ComputerName $ComputerName
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Add-XxXNetGroupUser -UserName john -GroupName Administrators
        
        Adds a localuser "john" to the local group "Administrators"

    .EXAMPLE

        PS C:\> Add-XxXNetGroupUser -UserName john -GroupName "Domain Admins" -Domain dev.local
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXUserProperty -Domain testing
        
        Returns all user properties for users in the 'testing' domain.

    .EXAMPLE

        PS C:\> Get-XxXUserProperty -Properties ssn,lastlogon,location
...
#>
    ...
    if($Properties) {
        ...
        Get-XxXNetUser -Domain $Domain -DomainController $DomainController -PageSize $PageSize -Credential $Credential | Select-Object -Property $Properties
    }
    else {
        # extract out just the property names
        Get-XxXNetUser -Domain $Domain -DomainController $DomainController -PageSize $PageSize -Credential $Credential | Select-Object -First 1 | Get-Member -MemberType *Property | Select-Object -Property 'Name'
    }
}


filter Find-XxXUserField {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXUserField -SearchField info -SearchTerm backup
...
#>
    ...
    Get-XxXNetUser -ADSpath $ADSpath -Domain $Domain -DomainController $DomainController -Credential $Credential -Filter "($SearchField=*$SearchTerm*)" -PageSize $PageSize | Select-Object samaccountname,$SearchField
}


filter Get-XxXUserEvent {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXUserEvent -ComputerName DomainController.testlab.local
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXObjectAcl -SamAccountName matt.admin -domain testlab.local
        
        Get the ACLs for the matt.admin user in the testlab.local domain

    .EXAMPLE

        PS C:\> Get-XxXObjectAcl -SamAccountName matt.admin -domain testlab.local -ResolveGUIDs
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetOU -FullData | Get-XxXObjectAcl -ResolveGUIDs
...
#>
    ...
    begin {
        $Searcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -ADSpath $ADSpath -ADSprefix $ADSprefix -PageSize $PageSize 

        # get a GUID -> name mapping
        if($ResolveGUIDs) {
            $GUIDs = Get-XxXGUIDMap -Domain $Domain -DomainController $DomainController -PageSize $PageSize
        }
    }
...
<#
    ...
    .EXAMPLE
        ...
        Add-XxXObjectAcl -TargetSamAccountName matt -PrincipalSamAccountName john

        Grants 'john' all full access rights to the 'matt' account.

    .EXAMPLE

        Add-XxXObjectAcl -TargetSamAccountName matt -PrincipalSamAccountName john -Rights ResetPassword
...
#>
    ...
    begin {
        $Searcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -ADSpath $TargetADSpath -ADSprefix $TargetADSprefix -PageSize $PageSize

        if($PrincipalSID) {
            $ResolvedPrincipalSID = $PrincipalSID
        }
        else {
            $Principal = Get-XxXADObject -Domain $Domain -DomainController $DomainController -Name $PrincipalName -SamAccountName $PrincipalSamAccountName -PageSize $PageSize
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXACLScanner -ResolveGUIDs | Export-CSV -NoTypeInformation acls.csv
...
#>
    ...
    $SchemaPath = (Get-XxXNetworkForest).schema.name

    $SchemaSearcher = Get-XxXDomainSearcher -ADSpath $SchemaPath -DomainController $DomainController -PageSize $PageSize
    if($SchemaSearcher) {
    ...
    $RightsSearcher = Get-XxXDomainSearcher -ADSpath $SchemaPath.replace("Schema","Extended-Rights") -DomainController $DomainController -PageSize $PageSize -Credential $Credential
    if ($RightsSearcher) {
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetComputer
        
        Returns the current computers in current domain.

    .EXAMPLE

        PS C:\> Get-XxXNetComputer -SPN mssql*
        
        Returns all MS SQL servers on the domain.

    .EXAMPLE

        PS C:\> Get-XxXNetComputer -Domain testing
        
        Returns the current computers in 'testing' domain.

    .EXAMPLE

        PS C:\> Get-XxXNetComputer -Domain testing -FullData
...
#>
    ...
    begin {
        ...
        $CompSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -ADSpath $ADSpath -PageSize $PageSize -Credential $Credential
    }
    ...
    process {
        ...
        if ($CompSearcher) {
            ...
            Write-Verbose "Get-XxXNetComputer filter : '$CompFilter'"
...
<#
    ...
    .PARAMETER ReturnRaw
        ...
        Used by Set-XxXADObject to modify object properties.
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXADObject -SID "S-1-5-21-2620891829-2411261497-1773853088-1110"
        
        Get the domain object associated with the specified SID.
        
    .EXAMPLE

        PS C:\> Get-XxXADObject -ADSpath "CN=AdminSDHolder,CN=System,DC=testlab,DC=local"
...
#>
    ...
    process {
        ...
        $ObjectSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $ADSpath -PageSize $PageSize
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Set-XxXADObject -SamAccountName matt.admin -PropertyName countrycode -PropertyValue 0
        
        Set the countrycode for matt.admin to 0

    .EXAMPLE

        PS C:\> Set-XxXADObject -SamAccountName matt.admin -PropertyName useraccountcontrol -PropertyXorValue 65536
...
#>
    ...
    # splat the appropriate arguments to Get-XxXADObject
    $RawObject = Get-XxXADObject -ReturnRaw @Arguments
...
function Invoke-XxXDowngradeAccount {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS> Invoke-XxXDowngradeAccount -SamAccountName jason

        Set reversible encryption on the 'jason' account and force the password to be changed.

    .EXAMPLE

        PS> Invoke-XxXDowngradeAccount -SamAccountName jason -Repair
...
#>
    ...
    process {
        ...
        # splat the appropriate arguments to Get-XxXADObject
        $UACValues = Get-XxXADObject @Arguments | select useraccountcontrol | ConvertFrom-UACValue

        if($Repair) {

            if($UACValues.Keys -contains "ENCRYPTED_TEXT_PWD_ALLOWED") {
                # if reversible encryption is set, unset it
                Set-XxXADObject @Arguments -PropertyName useraccountcontrol -PropertyXorValue 128
            }

            # unset the forced password change
            Set-XxXADObject @Arguments -PropertyName pwdlastset -PropertyValue -1
        }
            ...
            if($UACValues.Keys -contains "DONT_EXPIRE_PASSWORD") {
                ...
                Set-XxXADObject @Arguments -PropertyName useraccountcontrol -PropertyXorValue 65536
            }
            ...
            Set-XxXADObject @Arguments -PropertyName pwdlastset -PropertyValue 0
        }
    }
}


function Get-XxXComputerProperty {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXComputerProperty -Domain testing
        
        Returns all user properties for computers in the 'testing' domain.

    .EXAMPLE

        PS C:\> Get-XxXComputerProperty -Properties ssn,lastlogon,location
...
#>
    ...
    if($Properties) {
        ...
        Get-XxXNetComputer -Domain $Domain -DomainController $DomainController -Credential $Credential -FullData -PageSize $PageSize | Select-Object -Property $Properties
    }
    else {
        # extract out just the property names
        Get-XxXNetComputer -Domain $Domain -DomainController $DomainController -Credential $Credential -FullData -PageSize $PageSize | Select-Object -first 1 | Get-Member -MemberType *Property | Select-Object -Property "Name"
    }
}


function Find-XxXComputerField {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXComputerField -SearchTerm backup -SearchField info
...
#>
    ...
    process {
        Get-XxXNetComputer -ADSpath $ADSpath -Domain $Domain -DomainController $DomainController -Credential $Credential -FullData -Filter "($SearchField=*$SearchTerm*)" -PageSize $PageSize | Select-Object samaccountname,$SearchField
    }
}


function Get-XxXNetOU {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetOU -OUName *admin* -Domain testlab.local
        
        Returns all OUs with "admin" in their name in the testlab.local domain.

     .EXAMPLE

        PS C:\> Get-XxXNetOU -GUID 123-...
        
        Returns all OUs with linked to the specified group policy object.

     .EXAMPLE

        PS C:\> "*admin*","*server*" | Get-XxXNetOU
...
#>
    ...
    begin {
        $OUSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $ADSpath -PageSize $PageSize
    }
    process {
        if ($OUSearcher) {
            if ($GUID) {
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetSite -Domain testlab.local -FullData
...
#>
    ...
    begin {
        $SiteSearcher = Get-XxXDomainSearcher -ADSpath $ADSpath -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSprefix "CN=Sites,CN=Configuration" -PageSize $PageSize
    }
    process {
        if($SiteSearcher) {
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetSubnet
        
        Returns all subnet names in the current domain.

    .EXAMPLE

        PS C:\> Get-XxXNetSubnet -Domain testlab.local -FullData
...
#>
    ...
    begin {
        $SubnetSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $ADSpath -ADSprefix "CN=Subnets,CN=Sites,CN=Configuration" -PageSize $PageSize
    }
...
<#
    ...
    .EXAMPLE
        ...
        C:\> Get-XxXDomainSID -Domain TEST
...
#>
    ...
    $DCSID = Get-XxXNetComputer -Domain $Domain -DomainController $DomainController -FullData -Filter '(userAccountControl:1.2.840.113556.1.4.803:=8192)' | Select-Object -First 1 -ExpandProperty objectsid
    if($DCSID) {
...
<#
    ...
    .PARAMETER RawSids
        ...
        Switch. Return raw SIDs when using "Get-XxXNetworkGroup -UserName X"
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetworkGroup

        Returns the current security groups in the domain.

    .EXAMPLE

        PS C:\> Get-XxXNetworkGroup -GroupName *admin*

        Returns all groups with "admin" in their group name.

    .EXAMPLE

        PS C:\> Get-XxXNetworkGroup -Domain testing -FullData
...
#>
    ...
    begin {
        $GroupSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $ADSpath -PageSize $PageSize
        if (!$AllTypes)
        {
    ...
    process {
        if($GroupSearcher) {
            ...
            if ($UserName) {
                ...
                $User = Get-XxXADObject -SamAccountName $UserName -Domain $Domain -DomainController $DomainController -Credential $Credential -ReturnRaw -PageSize $PageSize | Select-Object -First 1
                ...
                if($User) {
                    ...
                    $UserDirectoryEntry.TokenGroups | ForEach-Object {
                        ...
                        if($GroupSid -notmatch '^S-1-5-32-.*') {
                            if($FullData) {
                                $Group = Get-XxXADObject -SID $GroupSid -PageSize $PageSize -Domain $Domain -DomainController $DomainController -Credential $Credential
...
function Get-XxXNetGroupMember {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetGroupMember
        
        Returns the usernames that of members of the "Domain Admins" domain group.

    .EXAMPLE

        PS C:\> Get-XxXNetGroupMember -Domain testing -GroupName "Power Users"
...
#>
    ...
    begin {
        if($DomainController) {
            ...
            $TargetDomainController = ((Get-XxXNetworkDomain -Credential $Credential).PdcRoleOwner).Name
        }
        ...
        if($Domain) {
            ...
            $TargetDomain = Get-XxXNetworkDomain -Credential $Credential | Select-Object -ExpandProperty name
        }

        # so this isn't repeated if users are passed on the pipeline
        $GroupSearcher = Get-XxXDomainSearcher -Domain $TargetDomain -DomainController $TargetDomainController -Credential $Credential -ADSpath $ADSpath -PageSize $PageSize
    }
    ...
    process {
        if ($GroupSearcher) {
            if ($Recurse -and $UseMatchingRule) {
                ...
                if ($GroupName) {
                    $Group = Get-XxXNetworkGroup -AllTypes -GroupName $GroupName -Domain $TargetDomain -DomainController $TargetDomainController -Credential $Credential -FullData -PageSize $PageSize
                }
                elseif ($SID) {
                    $Group = Get-XxXNetworkGroup -AllTypes -SID $SID -Domain $TargetDomain -DomainController $TargetDomainController -Credential $Credential -FullData -PageSize $PageSize
                }
                else {
                    # default to domain admins
                    $SID = (Get-XxXDomainSID -Domain $TargetDomain -DomainController $TargetDomainController) + "-512"
                    $Group = Get-XxXNetworkGroup -AllTypes -SID $SID -Domain $TargetDomain -DomainController $TargetDomainController -Credential $Credential -FullData -PageSize $PageSize
                }
            ...
            $Members | Where-Object {$_} | ForEach-Object {
                ...
                if($Properties) {
                    ...
                    if ($Recurse -and !$UseMatchingRule -and $IsGroup -and $MemberName) {
                        if($FullData) {
                            Get-XxXNetGroupMember -FullData -Domain $MemberDomain -DomainController $TargetDomainController -Credential $Credential -GroupName $MemberName -Recurse -PageSize $PageSize
                        }
                        else {
                            Get-XxXNetGroupMember -Domain $MemberDomain -DomainController $TargetDomainController -Credential $Credential -GroupName $MemberName -Recurse -PageSize $PageSize
                        }
                    }
                }
            }
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetFileServer
        
        Returns active file servers.

    .EXAMPLE

        PS C:\> Get-XxXNetFileServer -Domain testing
...
#>
    ...
    Get-XxXNetUser -Domain $Domain -DomainController $DomainController -Credential $Credential -PageSize $PageSize -Filter $filter | Where-Object {$_} | Where-Object {
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXDFSshare

        Returns all distributed file system shares for the current domain.

    .EXAMPLE

        PS C:\> Get-XxXDFSshare -Domain test
...
#>
    ...
    function Get-XxXDFSshareV1 {
        ...
        $DFSsearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $ADSpath -PageSize $PageSize
        ...
        if($DFSsearcher) {
            ...
            try {
                ...
                Write-Warning "Get-XxXDFSshareV1 error : $_"
            }
            $DFSshares | Sort-Object -Property "RemoteServerName"
        }
    }

    function Get-XxXDFSshareV2 {
        [CmdletBinding()]
        param(
            [String]
            $Domain,
        ...
        if($DFSsearcher) {
            ...
            try {
                ...
                Write-Warning "Get-XxXDFSshareV2 error : $_"
            }
    ...
    if ( ($Version -eq "all") -or ($Version.endsWith("1")) ) {
        $DFSshares += Get-XxXDFSshareV1 -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $ADSpath -PageSize $PageSize
    }
    if ( ($Version -eq "all") -or ($Version.endsWith("2")) ) {
        $DFSshares += Get-XxXDFSshareV2 -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $ADSpath -PageSize $PageSize
    }
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXGptTmpl -GptTmplPath "\\dev.testlab.local\sysvol\dev.testlab.local\Policies\{31B2F340-016D-11D2-945F-00C04FB984F9}\MACHINE\Microsoft\Windows NT\SecEdit\GptTmpl.inf"
...
#>
    ...
    try {
        ...
        $TargetGptTmplPath | Get-XxXIniContent -ErrorAction SilentlyContinue
    }
    catch {
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetworkGPO -Domain testlab.local
...
#>
    ...
    begin {
        $GPOSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $ADSpath -PageSize $PageSize
    }
    ...
    process {
        if ($GPOSearcher) {
            ...
            if($ComputerName) {
                $GPONames = @()
                $Computers = Get-XxXNetComputer -ComputerName $ComputerName -Domain $Domain -DomainController $DomainController -FullData -PageSize $PageSize
                ...
                ForEach($ComputerOU in $ComputerOUs) {
                    $GPONames += Get-XxXNetOU -Domain $Domain -DomainController $DomainController -ADSpath $ComputerOU -FullData -PageSize $PageSize | ForEach-Object { 
                        # get any GPO links
                ...
                $ComputerSite = (Get-XxXSiteName -ComputerName $ComputerName).SiteName
                if($ComputerSite -and ($ComputerSite -notlike 'Error*')) {
                    $GPONames += Get-XxXNetSite -SiteName $ComputerSite -FullData | ForEach-Object {
                        if($_.gplink) {
                            $_.gplink.split("][") | ForEach-Object {
                                if ($_.startswith("LDAP")) {
                                    $_.split(";")[0]
                ...
                $GPONames | Where-Object{$_ -and ($_ -ne '')} | ForEach-Object {
                    ...
                    $GPOSearcher = Get-XxXDomainSearcher -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $_ -PageSize $PageSize
...
function New-XxXGPOImmediateTask {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS> New-XxXGPOImmediateTask -TaskName Debugging -GPODisplayName SecurePolicy -CommandArguments '-c "123 | Out-File C:\Temp\debug.txt"' -Force
    ...
    .EXAMPLE
        ...
        PS> New-XxXGPOImmediateTask -GPODisplayName SecurePolicy -Remove -Force
...
#>
    ...
    $GPOs = Get-XxXNetworkGPO -GPOname $GPOname -DisplayName $GPODisplayName -Domain $Domain -DomainController $DomainController -ADSpath $ADSpath -Credential $Credential 
...
<#
    .SYNOPSIS
        ...
        Author: @harmj0y
        License: BSD 3-Clause
        Required Dependencies: Get-XxXNetworkGPO, Get-XxXGptTmpl, Get-XxXGroupsXML, Convert-NameToSid, Convert-SidToName
        Optional Dependencies: None
    
    .DESCRIPTION

        First enumerates all GPOs in the current/target domain using Get-XxXNetworkGPO with passed
        ...
        GptTmpl.inf files found, the file is parsed with Get-XxXGptTmpl and any 'Group Membership'
        section data is processed if present. Any found Groups.xml files are parsed with
        Get-XxXGroupsXML and those memberships are returned as well.
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetGPOGroup

        Returns all local groups set by GPO along with their members and memberof.

    .EXAMPLE

        PS C:\> Get-XxXNetGPOGroup -ResolveMemberSIDs
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetGPOGroup -GPOName '{0847C615-6C4E-4D45-A064-6001040CC21C}'

        Return any GPO-set groups for the GPO with the given name/GUID.

    .EXAMPLE

        PS C:\> Get-XxXNetGPOGroup -DisplayName 'Desktops'
...
#>
    ...
    Get-XxXNetworkGPO -GPOName $GPOname -DisplayName $DisplayName -Domain $Domain -DomainController $DomainController -ADSpath $ADSpath -PageSize $PageSize | ForEach-Object {
        ...
        Get-XxXGroupsXML @ParseArgs | ForEach-Object {
            if($ResolveMemberSIDs) {
                $GroupMembers = @()
...
<#
    .SYNOPSIS
        ...
        Author: @harmj0y
        License: BSD 3-Clause
        Required Dependencies: Get-XxXNetUser, Get-XxXNetworkGroup, Get-XxXNetGPOGroup, Get-XxXNetOU, Get-XxXNetComputer, Get-XxXADObject, Get-XxXNetSite
        Optional Dependencies: None
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXGPOLocation -UserName dfm
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXGPOLocation -UserName dfm -Domain dev.testlab.local
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXGPOLocation -UserName jason -LocalGroup RDP
...
#>
    ...
    if($UserName) {
        ...
        $User = Get-XxXNetUser -UserName $UserName -Domain $Domain -DomainController $DomainController -PageSize $PageSize | Select-Object -First 1
    ...
    elseif($GroupName) {
        ...
        $Group = Get-XxXNetworkGroup -GroupName $GroupName -Domain $Domain -DomainController $DomainController -FullData -PageSize $PageSize | Select-Object -First 1
    ...
    # if we're not listing all relationships, use the tokenGroups approach from Get-XxXNetworkGroup to 
    # get all effective security SIDs this object is a part of
    if($TargetSIDs[0] -and ($TargetSIDs[0] -ne '*')) {
        $TargetSIDs += Get-XxXNetworkGroup -Domain $Domain -DomainController $DomainController -PageSize $PageSize -UserName $ObjectSamAccountName -RawSids
    }
    ...
    $GPOgroups = Get-XxXNetGPOGroup @GPOGroupArgs | ForEach-Object {
    ...
    $GPOgroups | ForEach-Object {
        ...
        Get-XxXNetOU -Domain $Domain -DomainController $DomainController -GUID $GPOguid -FullData -PageSize $PageSize | ForEach-Object {
            if($Filters) {
                # filter for computer name/org unit if a filter is specified
                #   TODO: handle other filters (i.e. OU filters?) again, I hate you GPP...
                $OUComputers = Get-XxXNetComputer -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $_.ADSpath -FullData -PageSize $PageSize | Where-Object {
                    $_.adspath -match ($Filters.Value)
                } | ForEach-Object { $_.dnshostname }
            }
            else {
                $OUComputers = Get-XxXNetComputer -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $_.ADSpath -PageSize $PageSize
            }
            ...
            if($OUComputers) {
                ...
                ForEach ($TargetSid in $TargetObjectSIDs) {
                    $Object = Get-XxXADObject -SID $TargetSid -Domain $Domain -DomainController $DomainController -Credential $Credential -PageSize $PageSize
        ...
        Get-XxXNetSite -Domain $Domain -DomainController $DomainController -GUID $GPOguid -PageSize $PageSize -FullData | ForEach-Object {
...
function Find-XxXGPOComputerAdmin {
<#
    .SYNOPSIS
        ...
        Author: @harmj0y
        License: BSD 3-Clause
        Required Dependencies: Get-XxXNetComputer, Get-XxXSiteName, Get-XxXNetSite, Get-XxXNetGPOGroup, Get-XxXADObject, Get-XxXNetGroupMember, Convert-SidToName
        Optional Dependencies: None
    ...
    .DESCRIPTION
        ...
        Get-XxXSiteName and retrieve all sites object Get-XxXNetSite. For those results, attempt to
        enumerate all linked GPOs and associated local group settings with Get-XxXNetGPOGroup. For
        ...
        Inverse of Find-XxXGPOLocation.
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXGPOComputerAdmin -ComputerName WINDOWS3.dev.testlab.local
        
        Finds users who have local admin rights over WINDOWS3 through GPO correlation.

    .EXAMPLE

        PS C:\> Find-XxXGPOComputerAdmin -ComputerName WINDOWS3.dev.testlab.local -LocalGroup RDP
...
#>
    ...
    process {
        ...
        if($ComputerName) {
            ...
            $ComputerSite = (Get-XxXSiteName -ComputerName $ComputerName).SiteName
            if($ComputerSite -and ($ComputerSite -notlike 'Error*')) {
                $GPOGroups += Get-XxXNetSite -SiteName $ComputerSite -FullData | ForEach-Object {
                    if($_.gplink) {
                        $_.gplink.split("][") | ForEach-Object {
                            if ($_.startswith("LDAP")) {
                                $_.split(";")[0]
                    ...
                    Get-XxXNetGPOGroup @GPOGroupArgs
                }
            }
        }
        else {
        ...
        $TargetOUs | Where-Object {$_} | ForEach-Object {
            ...
            $GPOLinks = Get-XxXNetOU -Domain $Domain -DomainController $DomainController -ADSpath $_ -FullData -PageSize $PageSize | ForEach-Object { 
        ...
        $GPOgroups | Sort-Object -Property GPOName -Unique | ForEach-Object {
            ...
            $GPOMembers | ForEach-Object {
                ...
                $Object = Get-XxXADObject -Domain $Domain -DomainController $DomainController -PageSize $PageSize -SID $_
                ...
                if($Recurse -and $GPOComputerAdmin.isGroup) {
                    ...
                    Get-XxXNetGroupMember -Domain $Domain -DomainController $DomainController -SID $_ -FullData -Recurse -PageSize $PageSize | ForEach-Object {
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXDomainPolicy

        Returns the domain policy for the current domain. 

    .EXAMPLE

        PS C:\> Get-XxXDomainPolicy -Source DC -DomainController MASTER.testlab.local
...
#>
    ...
    if($Source -eq "Domain") {
        ...
        $GPO = Get-XxXNetworkGPO -Domain $Domain -DomainController $DomainController -GPOname "{31B2F340-016D-11D2-945F-00C04FB984F9}"
        ...
        if($GPO) {
            ...
            Get-XxXGptTmpl @ParseArgs
        }

    }
    elseif($Source -eq "DC") {
        # query the given domain/dc for the default domain controller policy object
        $GPO = Get-XxXNetworkGPO -Domain $Domain -DomainController $DomainController -GPOname "{6AC1786C-016F-11D2-945F-00C04FB984F9}"
        ...
        if($GPO) {
            ...
            # parse the GptTmpl.inf
            Get-XxXGptTmpl @ParseArgs | ForEach-Object {
                if($ResolveSids) {
...
<#
    ...
    .PARAMETER ListGroups
        ...
        Old Get-XxXNetLocalGroups functionality.
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetLocalGroup -ComputerName WINDOWS7 -ListGroups

        Returns all local groups on the WINDOWS7 host.

    .EXAMPLE

        PS C:\> "WINDOWS7", "WINDOWSSP" | Get-XxXNetLocalGroup -API
...
#>
    ...
    process {
        ...
        if($ComputerFile) {
            ...
            $Servers += $ComputerName | Get-XxXNameField
        }
                        ...
                        $Members | ForEach-Object {
                            ...
                            if($Recurse -and $IsGroup) {
                                if($IsDomain) {
                                  ...
                                  Get-XxXNetGroupMember -GroupName $GroupName -Domain $FQDN -FullData -Recurse | ForEach-Object {
                                ...
                                Get-XxXNetLocalGroup -ComputerName $Server -GroupName $ObjName -Recurse
                              }
                            }
                        }
                    }
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetShare

        Returns active shares on the local host.

    .EXAMPLE

        PS C:\> Get-XxXNetShare -ComputerName sqlserver

        Returns active shares on the 'sqlserver' host

    .EXAMPLE

        PS C:\> Get-XxXNetComputer | Get-XxXNetShare
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetLoggedon

        Returns users actively logged onto the local host.

    .EXAMPLE

        PS C:\> Get-XxXNetLoggedon -ComputerName sqlserver

        Returns users actively logged onto the 'sqlserver' host.

    .EXAMPLE

        PS C:\> Get-XxXNetComputer | Get-XxXNetLoggedon
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetSession

        Returns active sessions on the local host.

    .EXAMPLE

        PS C:\> Get-XxXNetSession -ComputerName sqlserver

        Returns active sessions on the 'sqlserver' host.

    .EXAMPLE

        PS C:\> Get-XxXNetDomainController | Get-XxXNetSession
...
<#
    .SYNOPSIS
        ...
        using the NetWkstaUserEnum API (Get-XxXNetLoggedon) of less user privileges
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXLoggedOnLocal

        Returns active sessions on the local host.

    .EXAMPLE

        PS C:\> Get-XxXLoggedOnLocal -ComputerName sqlserver
...
#>
    ...
    $ComputerName = Get-XxXNameField -Object $ComputerName
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetRDPSession

        Returns active RDP/terminal sessions on the local host.

    .EXAMPLE

        PS C:\> Get-XxXNetRDPSession -ComputerName "sqlserver"

        Returns active RDP/terminal sessions on the 'sqlserver' host.

    .EXAMPLE

        PS C:\> Get-XxXNetDomainController | Get-XxXNetRDPSession
...
filter Invoke-XxXCheckLocalAdminAccess {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXCheckLocalAdminAccess -ComputerName sqlserver

        Returns active sessions on the local host.

    .EXAMPLE

        PS C:\> Get-XxXNetComputer | Invoke-XxXCheckLocalAdminAccess
...
#>
    ...
    Write-Verbose "Invoke-XxXCheckLocalAdminAccess handle: $Handle"
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXSiteName -ComputerName WINDOWS1

        Returns the site for WINDOWS1.testlab.local.

    .EXAMPLE

        PS C:\> Get-XxXNetComputer | Get-XxXSiteName
...
#>
    ...
    if($Computer -match '^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$') {
        ...
        $IPAddress = @(Get-XxXIPAddress -ComputerName $Computer)[0].IPAddress
    }
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXLastLoggedOn

        Returns the last user logged onto the local machine.

    .EXAMPLE
        
        PS C:\> Get-XxXLastLoggedOn -ComputerName WINDOWS1

        Returns the last user logged onto WINDOWS1

    .EXAMPLE
        
        PS C:\> Get-XxXNetComputer | Get-XxXLastLoggedOn
...
filter Get-XxXCachedRDPConnection {
<#
    .SYNOPSIS
...
filter Get-XxXRegistryMountedDrive {
<#
    .SYNOPSIS
...
<#
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetProcess -ComputerName WINDOWS1
...
function Find-XxXInterestingFile {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXInterestingFile -Path C:\Backup\
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXInterestingFile -Path \\WINDOWS7\Users\ -Terms salaries,email -OutFile out.csv
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXInterestingFile -Path \\WINDOWS7\Users\ -LastAccessTime (Get-Date).AddDays(-7)
...
function Invoke-XxXThreadedFunction {
...
<#
    ...
    .DESCRIPTION
        ...
        This function finds the local domain name for a host using Get-XxXNetworkDomain,
        queries the domain for users of a specified group (default "domain admins")
        with Get-XxXNetGroupMember or reads in a target user list, queries the domain for all
        active machines with Get-XxXNetComputer or reads in a pre-populated host list,
        randomly shuffles the target list, then for each server it gets a list of
        active users with Get-XxXNetSession/Get-XxXNetLoggedon. The found user list is compared
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXUserHunter -CheckAccess
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXUserHunter -Domain 'testing'

        Finds machines on the 'testing' domain where domain admins are logged into.

    .EXAMPLE

        PS C:\> Invoke-XxXUserHunter -Threads 20

        Multi-threaded user hunting, replaces Invoke-XxXUserHunterThreaded.

    .EXAMPLE

        PS C:\> Invoke-XxXUserHunter -UserFile users.txt -ComputerFile hosts.txt
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXUserHunter -GroupName "Power Users" -Delay 60
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXUserHunter -TargetServer FILESERVER

        Query FILESERVER for useres who are effective local administrators using
        Get-XxXNetLocalGroup -Recurse, and hunt for that user set on the network.

    .EXAMPLE

        PS C:\> Invoke-XxXUserHunter -SearchForest

        Find all machines in the current forest where domain admins are logged in.

    .EXAMPLE

        PS C:\> Invoke-XxXUserHunter -Stealth

        Executes old Invoke-XxXStealthUserHunter functionality, enumerating commonly
        used servers and checking just sessions for each.

    .EXAMPLE

        PS C:\> Invoke-XxXUserHunter -Stealth -StealthSource DC -Poll 3600 -Delay 5 -ShowAll | ? { ! $_.UserName.EndsWith('$') }
...
#>
    ...
    begin {
        ...
        Write-Verbose "[*] Running Invoke-XxXUserHunter with delay of $Delay"
        ...
        if(!$ComputerName) { 
            ...
            elseif($SearchForest) {
                ...
                $TargetDomains = Get-XxXNetForestDomain | ForEach-Object { $_.Name }
            }
            else {
                # use the local domain
                $TargetDomains = @( (Get-XxXNetworkDomain).name )
            }
            ...
            if($Stealth) {
                ...
                ForEach ($Domain in $TargetDomains) {
                    if (($StealthSource -eq "File") -or ($StealthSource -eq "All")) {
                        ...
                        $ComputerName += Get-XxXNetFileServer -Domain $Domain -DomainController $DomainController
                    }
                    if (($StealthSource -eq "DFS") -or ($StealthSource -eq "All")) {
                        Write-Verbose "[*] Querying domain $Domain for DFS Servers..."
                        $ComputerName += Get-XxXDFSshare -Domain $Domain -DomainController $DomainController | ForEach-Object {$_.RemoteServerName}
                    }
                    if (($StealthSource -eq "DC") -or ($StealthSource -eq "All")) {
                        Write-Verbose "[*] Querying domain $Domain for Domain Controllers..."
                        $ComputerName += Get-XxXNetDomainController -LDAP -Domain $Domain -DomainController $DomainController | ForEach-Object { $_.dnshostname}
                    }
                }
            }
            else {
                ForEach ($Domain in $TargetDomains) {
                    ...
                    $ComputerName += Get-XxXNetComputer @Arguments
                }
            }
        ...
        elseif($TargetServer) {
            ...
            $TargetUsers = Get-XxXNetLocalGroup $TargetServer -Recurse | Where-Object {(-not $_.IsGroup) -and $_.IsDomain } | ForEach-Object {
                $User = New-Object PSObject
        ...
        elseif($UserADSpath -or $UserFilter -or $AdminCount) {
            ForEach ($Domain in $TargetDomains) {
                ...
                $TargetUsers += Get-XxXNetUser @Arguments | ForEach-Object {
                    $User = New-Object PSObject
            ...
            ForEach ($Domain in $TargetDomains) {
                ...
                $TargetUsers += Get-XxXNetGroupMember -GroupName $GroupName -Domain $Domain -DomainController $DomainController
            }
        }
            ...
            if($Up) {
                ...
                Do {
                    if(!$DomainShortName) {
                        ...
                        $Sessions = Get-XxXNetSession -ComputerName $ComputerName
                        ForEach ($Session in $Sessions) {
                            ...
                            if (($UserName) -and ($UserName.trim() -ne '') -and (!($UserName -match $CurrentUser))) {
                                ...
                                $TargetUsers | Where-Object {$UserName -like $_.MemberName} | ForEach-Object {
                                    ...
                                    $IPAddress = @(Get-XxXIPAddress -ComputerName $ComputerName)[0].IPAddress
                                    ...
                                    if ($CheckAccess) {
                                        $Admin = Invoke-XxXCheckLocalAdminAccess -ComputerName $CName
                    ...
                    if(!$Stealth) {
                        ...
                        $LoggedOn = Get-XxXNetLoggedon -ComputerName $ComputerName
                            ...
                            if (($UserName) -and ($UserName.trim() -ne '')) {
                                ...
                                $TargetUsers | Where-Object {$UserName -like $_.MemberName} | ForEach-Object {
                                    ...
                                    if($Proceed) {
                                        ...
                                        # see if we're checking to see if we have local admin access on this machine
                                        if ($CheckAccess) {
                                            $Admin = Invoke-XxXCheckLocalAdminAccess -ComputerName $ComputerName
                                            $FoundUser | Add-Member Noteproperty 'LocalAdmin' $Admin.IsAdmin
                                        }
                                        else {
                                            $FoundUser | Add-Member Noteproperty 'LocalAdmin' $Null
    ...
    process {
        ...
        if($Threads) {
            ...
            # if we're using threading, kick off the script block with Invoke-XxXThreadedFunction
            $ScriptParams = @{
                'Ping' = $(-not $NoPing)
            ...
            Invoke-XxXThreadedFunction -ComputerName $ComputerName -ScriptBlock $HostEnumBlock -ScriptParameters $ScriptParams -Threads $Threads
        }
            ...
            if(-not $NoPing -and ($ComputerName.count -ne 1)) {
                ...
                $ComputerName = Invoke-XxXThreadedFunction -NoImports -ComputerName $ComputerName -ScriptBlock $Ping -Threads 100
            }
...
function Invoke-XxXStealthUserHunter {
    [CmdletBinding()]
    param(
        [Parameter(Position=0,ValueFromPipeline=$True)]
        [Alias('Hosts')]
    ...
    # kick off Invoke-XxXUserHunter with stealth options
    Invoke-XxXUserHunter -Stealth @PSBoundParameters
}


function Invoke-XxXProcessHunter {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXProcessHunter -Domain 'testing'
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXProcessHunter -Threads 20

        Multi-threaded process hunting, replaces Invoke-XxXProcessHunterThreaded.

    .EXAMPLE

        PS C:\> Invoke-XxXProcessHunter -UserFile users.txt -ComputerFile hosts.txt
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXProcessHunter -GroupName "Power Users" -Delay 60
...
#>
    ...
    begin {
        ...
        Write-Verbose "[*] Running Invoke-XxXProcessHunter with delay of $Delay"
        ...
        if(!$ComputerName) { 
            ...
            elseif($SearchForest) {
                # get ALL the domains in the forest to search
                $TargetDomains = Get-XxXNetForestDomain -DomainController $DomainController -Credential $Credential | ForEach-Object { $_.Name }
            }
            else {
                # use the local domain
                $TargetDomains = @( (Get-XxXNetworkDomain -Domain $Domain -Credential $Credential).name )
            }

            ForEach ($Domain in $TargetDomains) {
                Write-Verbose "[*] Querying domain $Domain for hosts"
                $ComputerName += Get-XxXNetComputer -Domain $Domain -DomainController $DomainController -Credential $Credential -Filter $ComputerFilter -ADSpath $ComputerADSpath
            }
        ...
        if(!$ProcessName) {
            ...
            elseif($UserADSpath -or $UserFilter) {
                ForEach ($Domain in $TargetDomains) {
                    Write-Verbose "[*] Querying domain $Domain for users"
                    $TargetUsers += Get-XxXNetUser -Domain $Domain -DomainController $DomainController -Credential $Credential -ADSpath $UserADSpath -Filter $UserFilter | ForEach-Object {
                        $_.samaccountname
                    }  | Where-Object {$_}
                }
            }
                ...
                ForEach ($Domain in $TargetDomains) {
                    Write-Verbose "[*] Querying domain $Domain for users of group '$GroupName'"
                    $TargetUsers += Get-XxXNetGroupMember -GroupName $GroupName -Domain $Domain -DomainController $DomainController -Credential $Credential| ForEach-Object {
                        $_.MemberName
                    }
                }
            }
        ...
        $HostEnumBlock = {
            ...
            if($Up) {
                ...
                $Processes = Get-XxXNetProcess -Credential $Credential -ComputerName $ComputerName -ErrorAction SilentlyContinue
...
function Invoke-XxXEventHunter {
<#
    .SYNOPSIS
...
#>
    ...
    begin {
        ...
        Write-Verbose "[*] Running Invoke-XxXEventHunter"
        ...
        elseif($SearchForest) {
            ...
            # use the local domain
            $TargetDomains = @( (Get-XxXNetworkDomain -Credential $Credential).name )
        }
        ...
        if(!$ComputerName) { 
            ...
            elseif($ComputerFilter -or $ComputerADSpath) {
                ...
                ForEach ($Domain in $TargetDomains) {
                    Write-Verbose "[*] Querying domain $Domain for domain controllers"
                    $ComputerName += Get-XxXNetDomainController -LDAP -Domain $Domain -DomainController $DomainController -Credential $Credential | ForEach-Object { $_.dnshostname}
                }
            }
        ...
        $HostEnumBlock = {
            ...
            if($Up) {
                # try to enumerate
                if($Credential) {
                    Get-XxXUserEvent -ComputerName $ComputerName -Credential $Credential -EventType 'all' -DateStart ([DateTime]::Today.AddDays(-$SearchDays)) | Where-Object {
                        # filter for the target user set
                        $TargetUsers -contains $_.UserName
                    }
                }
                else {
                    Get-XxXUserEvent -ComputerName $ComputerName -EventType 'all' -DateStart ([DateTime]::Today.AddDays(-$SearchDays)) | Where-Object {
                        # filter for the target user set
                        $TargetUsers -contains $_.UserName
                    }
                }
...
function Invoke-XxXShareFinder {
<#
    .SYNOPSIS

        This function finds the local domain name for a host using Get-XxXNetworkDomain,
        queries the domain for all active machines with Get-XxXNetComputer, then for
        each server it lists of active shares with Get-XxXNetShare. Non-standard shares
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXShareFinder -ExcludeStandard

        Find non-standard shares on the domain.

    .EXAMPLE

        PS C:\> Invoke-XxXShareFinder -Threads 20

        Multi-threaded share finding, replaces Invoke-XxXShareFinderThreaded.

    .EXAMPLE

        PS C:\> Invoke-XxXShareFinder -Delay 60
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXShareFinder -ComputerFile hosts.txt
...
#>
    ...
    begin {
        ...
        Write-Verbose "[*] Running Invoke-XxXShareFinder with delay of $Delay"
        ...
        if(!$ComputerName) { 
            ...
            ForEach ($Domain in $TargetDomains) {
                Write-Verbose "[*] Querying domain $Domain for hosts"
                $ComputerName += Get-XxXNetComputer -Domain $Domain -DomainController $DomainController -Filter $ComputerFilter -ADSpath $ComputerADSpath
            }
        ...
        $HostEnumBlock = {
            ...
            if($Up) {
                ...
                $Shares = Get-XxXNetShare -ComputerName $ComputerName
...
<#
    ...
    .DESCRIPTION
        ...
        This function finds the local domain name for a host using Get-XxXNetworkDomain,
        queries the domain for all active machines with Get-XxXNetComputer, grabs
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXFileFinder -Domain testing
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXFileFinder -IncludeC
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXFileFinder -ShareList shares.txt -Terms accounts,ssn -OutFile out.csv
...
#>
    ...
    begin {
        ...
        Write-Verbose "[*] Running Invoke-XxXFileFinder with delay of $Delay"
        ...
        if($ShareList) {
            ForEach ($Item in Get-Content -Path $ShareList) {
                if (($Item -ne $Null) -and ($Item.trim() -ne '')) {
                    # exclude any "[tab]- commants", i.e. the output from Invoke-XxXShareFinder
            ...
            if(!$ComputerName) {
                ...
                if($SearchSYSVOL) {
                    ...
                    ForEach ($Domain in $TargetDomains) {
                        Write-Verbose "[*] Querying domain $Domain for hosts"
                        $ComputerName += Get-XxXNetComputer -Filter $ComputerFilter -ADSpath $ComputerADSpath -Domain $Domain -DomainController $DomainController
                    }
        ...
        $HostEnumBlock = {
            ...
            ForEach($Share in $SearchShares) {
                ...
                Find-XxXInterestingFile @SearchArgs
            }
        }
    }
    ...
    process {
        ...
        if($Threads) {
            ...
            if($Shares) {
                ...
                Invoke-XxXThreadedFunction -ComputerName $Shares -ScriptBlock $HostEnumBlock -ScriptParameters $ScriptParams -Threads $Threads
            }
            else {
                Invoke-XxXThreadedFunction -ComputerName $ComputerName -ScriptBlock $HostEnumBlock -ScriptParameters $ScriptParams -Threads $Threads
            }
...
function Find-XxXLocalAdminAccess {
<#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        access using Invoke-XxXCheckLocalAdminAccess.
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXLocalAdminAccess -Threads 10

        Multi-threaded access hunting, replaces Find-XxXLocalAdminAccessThreaded.

    .EXAMPLE

        PS C:\> Find-XxXLocalAdminAccess -Domain testing
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXLocalAdminAccess -ComputerFile hosts.txt
...
#>
    ...
    begin {
        ...
        Write-Verbose "[*] Running Find-XxXLocalAdminAccess with delay of $Delay"
        ...
        $HostEnumBlock = {
            ...
            if($Up) {
                ...
                $Access = Invoke-XxXCheckLocalAdminAccess -ComputerName $ComputerName
                if ($Access.IsAdmin) {
                    $ComputerName
                }
            }
...
function Get-XxXExploitableSystem {
<#
    .Synopsis
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXExploitableSystem -DomainController 192.168.1.1 -Credential demo.com\user | Format-Table -AutoSize
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXExploitableSystem | Export-Csv c:\temp\output.csv -NoTypeInformation

        How to write the output to a csv file.

    .EXAMPLE

        PS C:\> Get-XxXExploitableSystem -Domain testlab.local -Ping
     ...
     .LINK
       ...
       http://www.netspi.com
       https://github.com/nullbind/Powershellery/blob/master/Stable-ish/ADS/Get-XxXExploitableSystems.psm1
       
     .NOTES
       
       Author:  Scott Sutherland - 2015, NetSPI
                Modifications to integrate into PowerView by @harmj0y
       Version: Get-XxXExploitableSystem.psm1 v1.1
...
#>
    ...
    Get-XxXNetComputer -FullData @PSBoundParameters | ForEach-Object {
...
function Invoke-XxXEnumerateLocalAdmin {
<#
    .SYNOPSIS

        This function queries the domain for all active machines with
        Get-XxXNetComputer, then for each server it queries the local
        Administrators with Get-XxXNetLocalGroup.
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXEnumerateLocalAdmin
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXEnumerateLocalAdmin -Threads 10

        Threaded local admin enumeration, replaces Invoke-XxXEnumerateLocalAdminThreaded
...
#>
    ...
    begin {
        ...
        Write-Verbose "[*] Running Invoke-XxXEnumerateLocalAdmin with delay of $Delay"
        ...
        if($TrustGroups) {
            ...
            $TrustGroupNames = Find-XxXForeignGroup -Domain $Domain -DomainController $DomainController | ForEach-Object { $_.GroupName } | Sort-Object -Unique
            ...
            $TrustGroupsSIDs = $TrustGroupNames | ForEach-Object { 
                ...
                Get-XxXNetworkGroup -Domain $Domain -DomainController $DomainController -GroupName $_ -FullData | Where-Object { $_.objectsid -notmatch "S-1-5-32-544" } | ForEach-Object { $_.objectsid }
            }

            # query for the primary domain controller so we can extract the domain SID for filtering
            $DomainSID = Get-XxXDomainSID -Domain $Domain -DomainController $DomainController
        }
        ...
        $HostEnumBlock = {
            ...
            if($Up) {
                ...
                if($API) {
                    $LocalAdmins = Get-XxXNetLocalGroup -ComputerName $ComputerName -API
                }
                else {
                    $LocalAdmins = Get-XxXNetLocalGroup -ComputerName $ComputerName
                }
...
function Get-XxXNetDomainTrust {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetDomainTrust

        Return domain trusts for the current domain using built in .NET methods.

    .EXAMPLE

        PS C:\> Get-XxXNetDomainTrust -Domain "prod.testlab.local"

        Return domain trusts for the "prod.testlab.local" domain using .NET methods

    .EXAMPLE

        PS C:\> Get-XxXNetDomainTrust -LDAP -Domain "prod.testlab.local" -DomainController "PRIMARY.testlab.local"
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetDomainTrust -API -Domain "prod.testlab.local"

        Return domain trusts for the "prod.testlab.local" domain enumerated through API calls.

    .EXAMPLE

        PS C:\> Get-XxXNetDomainTrust -API -DomainController WINDOWS2.testlab.local
...
#>
    ...
    process {
        ...
        if(-not $Domain) {
            ...
            $SourceDomain = (Get-XxXNetworkDomain -Credential $Credential).Name
        }
        else {
            $SourceDomain = $Domain
        }
        ...
        if($LDAP -or $ADSPath) {
            ...
            $TrustSearcher = Get-XxXDomainSearcher -Domain $SourceDomain -DomainController $DomainController -Credential $Credential -PageSize $PageSize -ADSpath $ADSpath

            $SourceSID = Get-XxXDomainSID -Domain $SourceDomain -DomainController $DomainController
        ...
        elseif($API) {
            if(-not $DomainController) {
                $DomainController = Get-XxXNetDomainController -Credential $Credential -Domain $SourceDomain | Select-Object -First 1 | Select-Object -ExpandProperty Name
            }
...
function Get-XxXNetForestTrust {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Get-XxXNetForestTrust

        Return current forest trusts.

    .EXAMPLE

        PS C:\> Get-XxXNetForestTrust -Forest "test"
...
#>
    ...
    process {
        $FoundForest = Get-XxXNetworkForest -Forest $Forest -Credential $Credential
...
#>
    ...
    function Get-ForeignUser {
        ...
        Get-XxXNetUser -Domain $Domain -DomainController $DomainController -UserName $UserName -PageSize $PageSize -Filter '(memberof=*)' | ForEach-Object {
    ...
    if ($Recurse) {
        ...
        if($LDAP -or $DomainController) {
            $DomainTrusts = Invoke-XxXMapDomainTrust -LDAP -DomainController $DomainController -PageSize $PageSize | ForEach-Object { $_.SourceDomain } | Sort-Object -Unique
        }
        else {
            $DomainTrusts = Invoke-XxXMapDomainTrust -PageSize $PageSize | ForEach-Object { $_.SourceDomain } | Sort-Object -Unique
        }
...
#>
    ...
    function Get-ForeignGroup {
        ...
        Get-XxXNetworkGroup -GroupName $GroupName -Filter '(member=*)' -Domain $Domain -DomainController $DomainController -FullData -PageSize $PageSize | Where-Object {
...
function Find-XxXManagedSecurityGroups {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Find-XxXManagedSecurityGroups | Export-PowerViewCSV -NoTypeInformation group-managers.csv
...
#>
    ...
    Get-XxXNetworkGroup -FullData -Filter '(managedBy=*)' | Select-Object -Unique distinguishedName,managedBy,cn | ForEach-Object {

        # Retrieve the object that the managedBy DN refers to
        $group_manager = Get-XxXADObject -ADSPath $_.managedBy | Select-Object cn,distinguishedname,name,samaccounttype,samaccountname
        ...
        $xacl = Get-XxXObjectAcl -ADSPath $_.distinguishedname -Rights WriteMembers
...
function Invoke-XxXMapDomainTrust {
<#
    .SYNOPSIS
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-XxXMapDomainTrust | Export-CSV -NoTypeInformation trusts.csv
...
#>
    ...
    $CurrentDomain = (Get-XxXNetworkDomain -Credential $Credential).Name
    ...
    while($Domains.Count -ne 0) {
        ...
        if ($Domain -and ($Domain.Trim() -ne "") -and (-not $SeenDomains.ContainsKey($Domain))) {
            ...
            try {
                ...
                if($LDAP -or $DomainController) {
                    $Trusts = Get-XxXNetDomainTrust -Domain $Domain -LDAP -DomainController $DomainController -PageSize $PageSize -Credential $Credential
                }
                else {
                    $Trusts = Get-XxXNetDomainTrust -Domain $Domain -PageSize $PageSize -Credential $Credential
                }
                ...
                if(-not ($LDAP -or $DomainController) ) {
                    $Trusts += Get-XxXNetForestTrust -Forest $Domain -Credential $Credential
                }
